...
typedef struct {
    ...
    _Atomic int shutdown; // NNEKAS CHANGE
} crawler_ctx_t;
...
static void *worker_thread(void *arg)
{
    ...
    while (!ctx->shutdown) {
        ...
        /* Check page limit (race-free via atomic compare-and-swap style). */
        int fetched = atomic_load(&ctx->pages_fetched);
        ...
        if (depth < ctx->max_depth) {
            ...
            if (links) {
                for (int i = 0; i < links->count; i++) {
                    ...
                    links->urls[i] = NULL; /* take ownership */ //NNEKA MOVED THIS UP

                    // 🚫 ignore encoded / weird links
                    if (strchr(link, '%') != NULL) {
                        //free(link);
                        continue;
                    }

                    //🚫 FILTER BAD LINKS HERE
                    if (strncmp(link, "https://en.wikipedia.org/wiki/", 30) != 0 ||
                        strstr(link, "/wiki/Special") ||
                        strstr(link, "/wiki/Wikipedia") ||
                        strstr(link, "/wiki/Help") ||
                        strstr(link, "donate.wikimedia.org")) {
                        //free(link);
                        continue;
                    }
